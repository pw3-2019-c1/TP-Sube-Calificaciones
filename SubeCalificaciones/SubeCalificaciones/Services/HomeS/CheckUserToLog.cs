...
namespace SubeCalificaciones.Services.Home
{
    public class CheckUserToLog
    {
        ...
        public static Profesor GetProfesor(UsuarioAnonimo ua)
        {
            ...
            {
                Profesor profesorDetails = (from p in db.Profesor where p.Email == ua.InputEmail && p.Password == ua.InputPassword select p).First();
        ...
        public static Alumno GetAlumno(UsuarioAnonimo ua)
        {
            using (db = new TP_20191CEntities())
            {
                Alumno alumnoDetails = (from a in db.Alumno where a.Email == ua.InputEmail && a.Password == ua.InputPassword select a).First();
